...
let bin, bins = [], lineWidth, mapped, minDb, maxDb, lineWidths = [], phi;
createAudio();
addListeners();
...
function draw() {
	...
	for(i=0; i<settings.numCircles; i++) {
		...
		lineWidth = quadIn(t) * settings.radius;
	...
	for(let i = 0; i < settings.numCircles; i++) {
		...
		cradius += lineWidths[i]/2 + settings.gap;
...
function createAudio() {
	...
	audio.src = '../mp3/Apta - Elements 1 - 07 Breath-ii.mp3';
...
pane.addInput(settings, 'gap', { min: 0, max: 200, step: 1 })
